...
import type { InstagramStoryTemplateId } from "@/lib/instagram-story-templates";
...
function storyFeatures(property: Property, limit = 3) {
  return [
...
function StoryImage({ property }: { property: Property }) {
  if (property.image) {
    return (
      <img
        src={property.image}
  ...
  return (
    <div className="mx-auto w-full max-w-[330px] overflow-hidden rounded-2xl border border-[#E4E7EC] bg-[#EAECF0] shadow-sm">
      <div className="relative aspect-[9/16] overflow-hidden">
        <StoryImage property={property} />

        {templateId === "story-clean" && (
          <>
            <div className="absolute inset-0 bg-gradient-to-t from-black/75 via-transparent to-black/30" />
            <div className="absolute inset-x-5 top-[12%]">
              <StoryBrandMark brand={brand} inverse />
            </div>
            <div className="absolute inset-x-5 bottom-[19%] text-white">
        ...
        {templateId === "story-commercial" && (
          <>
            ...
            <div className="absolute inset-x-5 bottom-[17%] rounded-2xl bg-white/95 p-4 shadow-xl backdrop-blur">
              ...
              <FeaturePills items={storyFeatures(property, 3)} />
              <div
        ...
        {templateId === "story-opportunity" && (
          <>
            ...
            <div className="absolute inset-x-5 bottom-[18%] text-white">
              <div className="text-2xl font-black leading-tight">{headline}</div>
              <FeaturePills items={storyFeatures(property, 2)} inverse />
        ...
        {templateId === "story-branding" && (
          <>
            ...
            <div className="absolute left-6 top-[12%] rounded-xl bg-white/95 px-3 py-2 shadow">
              <StoryBrandMark brand={brand} />
            </div>
            <div className="absolute inset-x-6 bottom-[16%] rounded-2xl bg-white/95 p-4 shadow-xl backdrop-blur">
        ...
        <div className="pointer-events-none absolute inset-x-0 top-0 h-[11%] border-b border-dashed border-white/20" />
        <div className="pointer-events-none absolute inset-x-0 bottom-0 h-[15%] border-t border-dashed border-white/20" />
      </div>
      <div className="bg-white px-3 py-2 text-center text-[10px] font-semibold text-[#667085]">
        Prévia 9:16 · conteúdo mantido fora das zonas de interface
      </div>
    </div>
  );
}

export function InstagramStoryThumbnail({
  property,
  brand,
  templateId,
}: {
